...
const ProfileLink: React.FC<{ link: ProfileLink }> = ({ link }) => (
  <motion.div variants={itemAnimation}>
    <Button
      ...
      asChild
    >
      <Link to={`https://link-hub-api.vercel.app/api/profile/click/${link.shortUrl}`} target="_blank" rel="noopener noreferrer" className="flex items-center px-4">
...
const SocialIcons: React.FC<{ socials: SocialLinks }> = ({ socials }) => {
  ...
  const socialIcons = [
    { key: 'twitter', Icon: Twitter, url: getSocialIcon('twitter') !== '' ? `https://link-hub-api.vercel.app/api/profile/click/${getSocialIcon('twitter')}` : null },
    { key: 'facebook', Icon: Facebook, url: getSocialIcon('facebook') !== '' ? `https://link-hub-api.vercel.app/api/profile/click/${getSocialIcon('facebook')}` : null },
    { key: 'instagram', Icon: Instagram, url: getSocialIcon('instagram') !== '' ? `https://link-hub-api.vercel.app/api/profile/click/${getSocialIcon('instagram')}` : null },
    { key: 'spotify', Icon: Music2, url: getSocialIcon('spotify') !== '' ? `https://link-hub-api.vercel.app/api/profile/click/${getSocialIcon('spotify')}` : null },
    { key: 'twitch', Icon: Twitch, url: getSocialIcon('twitch') !== '' ? `https://link-hub-api.vercel.app/api/profile/click/${getSocialIcon('twitch')}` : null },
    { key: 'email', Icon: Mail, url: getSocialIcon('email') !== '' ? `https://link-hub-api.vercel.app/api/profile/click/${getSocialIcon('email')}` : null },
    { key: 'linkedin', Icon: Linkedin, url: getSocialIcon('linkedin') !== '' ? `https://link-hub-api.vercel.app/api/profile/click/${getSocialIcon('linkedin')}` : null },
    { key: 'github', Icon: GithubIcon, url: getSocialIcon('github') !== '' ? `https://link-hub-api.vercel.app/api/profile/click/${getSocialIcon('github')}` : null },
  ];
